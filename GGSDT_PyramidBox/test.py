...
os.environ["CUDA_VISIBLE_DEVICES"]='0,1'  # set cuda GPU index. For this code, we use two Titan XP 12G. Here 1 GPU is ok (1080ti).

print('Loading model..')
ssd_net = build_sfd('test', 640, 2)
net1 = ssd_net
net1.load_state_dict(torch.load('./GGSDT_PyramidBox/weights/Res50_pyramid_75000x.pth'))
netlist = [net1]  # netlist is used for multi-model testing, but not used here
net1.cuda()
net1.eval()
...
def detect_face(image, shrink, net=net1):
    ...
    x -= np.array([117, 119, 120],dtype=np.float32)  # here the means should be compatible to data
...
def multi_scale_test(image, max_im_shrink, net):
    ...
    if bt > 1:
        index = np.where(np.minimum(det_b[:, 2] - det_b[:, 0] + 1, det_b[:, 3] - det_b[:, 1] + 1) < 150)[0]
        det_b = det_b[index, :]
    else:
...
def multi_scale_test_pyramid(image, max_shrink, net):
    ...
    st = [0.5, 0.75, 1.25, 1.75, 2, 2.25, 2.75]
...
def bbox_vote(det):
    ...
    while det.shape[0] > 0:
        ...
        merge_index = np.where((o >= 0.35)+(inter==area[:]))[0]
    ...
    dets = dets[0:800, :]  # max number of heads in one picture
    return dets
...
if __name__ == '__main__':
    ...
    writepath = './GGSDT_PyramidBox/results/test_final_res.txt'
        ...
        with open(writepath,'w') as fw:
            for line in fr.readlines():
                ...
                for net in netlist:
                    ...
                    det4 = multi_scale_test_pyramid(image, max_im_shrink, net) # pyramid-scale test
                    det = np.row_stack((det, det0, det1, det2, det3, det4))
                    print('-'*5)

                dets = bbox_vote(det)  # NMS for results
