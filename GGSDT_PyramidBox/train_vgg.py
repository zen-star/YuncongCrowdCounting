...
from pyramid_vgg import build_vgg,SFD,SSHContext,ContextTexture  # when the backbone is vgg16
...
parser = argparse.ArgumentParser(description='PyramidBox-vgg16 YUNCONG Head Counting Training')
...
parser.add_argument('--annoPath', default="./yuncong_data/nice_zsd.txt", help='Location of wider face')  
...
ssd_net = build_vgg('train', 640, num_classes)
net = ssd_net
...
if not args.resume:
    ...
    ssd_net.load_weights('./GGSDT_PyramidBox/weights/vgg16bn.pth')
...
def train():
    ...
    for iteration in range(args.start_iter, max_iter):
        ...
        if iteration % 5000 == 0:
            ...
            torch.save(ssd_net.state_dict(), args.save_folder + 'Vgg16_pyramid_' +
                       repr(iteration) + '.pth')
    torch.save(ssd_net.state_dict(), args.save_folder + 'Vgg16_pyramid_' + '.pth')


def adjust_learning_rate(optimizer, gamma, step):
    # open this if training from checkpoint
	# if args.start_iter != 0: 
    #     return
    if step <= 2000:
        for param_group in optimizer.param_groups:
            param_group['lr'] = 2 * warmup * step
    elif step > 6000:
        if step % 100 == 0:
            for param_group in optimizer.param_groups:
