...
class Resize(object):
    ...
    def __call__(self, image, boxes=None, labels=None):
        interp_methods = [cv2.INTER_LINEAR, cv2.INTER_AREA, cv2.INTER_NEAREST, cv2.INTER_CUBIC, cv2.INTER_LANCZOS4]
        interp_method = random.choice(interp_methods)
        if(np.trace(image[:, :, 1]) == 0):
...
class RandomBaiduCrop(object):
    ...
    def __init__(self):
        ...
        self.mean = np.array([117, 119, 120],dtype=np.float32)
    ...
    def __call__(self, image, boxes=None, labels=None):
        ...
        if len(sample_boxes) > 0:
            ...
            #print('crop the box :',choice_:box)
            centers = (boxes[:, :2] + boxes[:, 2:]) / 2.0
...
class PhotometricDistort(object):
    ...
    def __call__(self, image, boxes, labels):
        ...
        im = image.copy()
        for method in self.pd:
            im,_,_ = method(im)
        #_convert(im, beta=random.uniform(-8,8)) 
...
def fix_abnormal(image, bbox, label, image0, bbox0, label0):
    ...
    bbox_mask = (bbox[:,2]-bbox[:,0] < 1.1) + (bbox[:,3]-bbox[:,1] < 1.1)
    if bbox_mask.any():
        print('After BaiduCrop, these bboxes are too small!')
